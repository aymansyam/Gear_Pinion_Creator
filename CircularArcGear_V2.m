...
%plot(dedendum_pt_st(1,:), dedendum_pt_st(2,:), 'LineStyle','--')
...
% Create matrix for whole wheel profile

rot_angle = 2 * pi / z_w;

for ii = 0 : z_w - 1

        temp_wheel =  rotation_z(-rot_angle * ii,  tooth_profile);

        x_wheel(1, ii * length(tooth_profile) + 1 : (ii + 1) * length(tooth_profile)) = temp_wheel(1,:);
        y_wheel(1, ii * length(tooth_profile) + 1 : (ii + 1) * length(tooth_profile)) = temp_wheel(2,:);

end

plot(x_wheel,y_wheel);


% ***************************** SHAFT ****************************************

[x_shaft, y_shaft] = calc_circle_pt(r_s,100);

plot(x_shaft,y_shaft);


% ***************************** CUTTED CIRCULAR SEGMENTS ****************************************

[x_inner_circle, y_inner_circle] = calc_circle_pt(r_s + r_s*0.45, 100);

%plot(x_inner_circle, y_inner_circle)

[x_outer_circle, y_outer_circle] = calc_circle_pt(hf_w_r - hf_w_r*0.1, 100);

%plot(x_outer_circle, y_outer_circle)

[inner_arc_x, inner_arc_y] = arc_circle(pi/3, 2*pi/3, 100, 0, 0, r_s + r_s*0.45);
%plot(inner_arc_x, inner_arc_y)

[outer_arc_x, outer_arc_y] = arc_circle(pi/3, 2*pi/3, 100, 0, 0, hf_w_r - hf_w_r*0.1);
%plot(outer_arc_x, outer_arc_y)

%plot([inner_arc_x ,flip(outer_arc_x), inner_arc_x(1,1)], [inner_arc_y, flip(outer_arc_y), inner_arc_y(1,1)]);
circular_segment = [inner_arc_x ,flip(outer_arc_x), inner_arc_x(1,1), NaN; inner_arc_y, flip(outer_arc_y), inner_arc_y(1,1), NaN];

%plot(circular_segment(1,:), circular_segment(2,:))

% Create 4 circular segments
rot_angle = 2 * pi / 4;

for ii = 0 : 4

        temp_segment =  rotation_z(-rot_angle * ii,  circular_segment);

%         x_cutout(1, ii * length(circular_segment) + 1 : (ii + 1) * length(circular_segment)) = temp_segment(1,:);
%         y_cutout(1, ii * length(circular_segment) + 1 : (ii + 1) * length(circular_segment)) = temp_segment(2,:);
